...
void loop()
{
    // give ADC a chance every time thru loop
    int pad;
    ...
    // parse command - sets mode and modeChanged
    if (done) cmdParse();
    ...
    {
        // run thru once - TODO - need multiple times if transition effect
        if (needInit)
        {
    ...
    switch (mode)
    {
       ...
       case LS_CLEAR:
        ...
        case SET_TRANSITION: // set transition at the next refresh - format TBD
            singleColor(mode);
            printSegments();
            mode = STOP_MODE;  // TODO - something else for transitions
            break;
        ...
        case ADC_MAX:
            getAdcStat(mode);
            mode = lastMode; // queries do not need to change display
            break;
        ...
        case LS_RANDOM_ADDRESS:
            ...
            {
                processErrors(LS_RANDOM_ADDRESS);
                mode = lastMode; // should not affect display
            }
            break;
    ...
    while (mySerial.available())
    {
        ...
        if (0 == newChar)
        {
            ...
            if (4 == syncCount) // resynchronize !
            {
                ...
                syncCount = 0;
                // char syncOut = 0x0;
                // mySerial.write(syncOut); // useful for debug, but message storm in system
...
// parse commandBytes in command processor - sets mode and modeChanged
bool cmdParse()
{
    bool done = false;
    // logChar = 0xCC; // complete command acquired

    lastMode = mode; // restore if command is not a mode change
...
void processSegmentCmd(unsigned char mode)
{
    ...
    switch (condx)
    {
        ...
        case CONDX_TRIG: target1 = triggerSegs; break;
    ...
    printSegments();
    //mySerial.write(target1, 3); // REMOVEME - show segment arrays
}
...
bool setRandomAddress()
{
    // first command byte is OK since the switch statement got us here

    // check second command byte 
    uint8_t cmd2 = commandBytes[2];
    if (LS_RANDOM_ADDRESS2 != cmd2)
        return false;

    // check three byte checksum
    if (!cmdChkSumOk(3))
        return false;

    //uint32_t kindaRnd = micros(); // phooey - takes 80 bytes more than using millis()
    //busAddress = kindaRnd & 0xf8;

    uint32_t kindaRnd = millis() & 0x1F; // keep 5 lowest bits
    busAddress = kindaRnd << 3;  // shift up to fit 0xF8 address mask

    // TODO - the new address evaporates at reset, could set the address in EEPROM

    dlog(busAddress); // greet with new address - disable since global address usually used
...
void eepromWrite()
{
    ...
    // EEPROM write is two byte command, one address byte, and one data byte, and checksum
    // The tile address and byte count is not checksummed
    uint8_t cmd2 = commandBytes[2];
    uint8_t addr = commandBytes[3];
    uint8_t datum = commandBytes[4];

    // first command byte is OK since the switch statement got us here

    // check second command byte 
    if (EEPROM_WRITE2 != cmd2)
        return processErrors(EEPROM_WRITE);

    // check five byte checksum
    if (!cmdChkSumOk(5))
        return processErrors(EEPROM_WRITE);

    // do not rewrite the same value
...
// check five byte checksum starting at first command byte
bool cmdChkSumOk(int count)
{
    uint8_t sum = 0;
